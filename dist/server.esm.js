...
import require$$0$2, { join, resolve } from 'path';
...
function destroyZlibStream (stream) {
  if (typeof stream.destroy === 'function') {
    ...
    } else if (stream._destroy && stream._destroy !== require$$1$2.Transform.prototype._destroy) {
...
function hasDestroy (stream) {
  return stream instanceof require$$1$2 &&
  ...
  return {
    ...
    config(config) {
      ...
      const getTemplate = name => {
        return readFileSync(resolve(process.cwd(), templateDir, name), {
          encoding: 'utf-8'
        });
      };
